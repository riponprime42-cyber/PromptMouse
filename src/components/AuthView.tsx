...
export function AuthView({ onBack, onSuccess }: AuthViewProps) {
  ...
  const handleGuestAuth = async () => {
    ...
    try {
      ...
      toast({ title: "Guest Access Initialized", description: "Welcome to the Studio vault." });
      onSuccess();
    } catch (error: any) {
      toast({
        title: "Guest Auth Error",
        description: error.message || "Failed to initialize guest session.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
        ...
        <CardContent className="p-10 space-y-8">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-primary/20 text-primary mb-4">
              {authMode === 'forgot' ? <KeyRound className="h-8 w-8" /> : <Sparkles className="h-8 w-8" />}
            </div>
            <h1 className="text-4xl font-black tracking-tighter">
              {authMode === 'login' ? "Welcome Back." : authMode === 'signup' ? "Create Identity." : "Reset Access."}
            </h1>
            <p className="text-white/40 text-sm font-light uppercase tracking-widest">
              {authMode === 'login' ? "Neural Credentials Required" : authMode === 'signup' ? "Register Your Neural Signature" : "Request Reset Protocol"}
            </p>
          </div>
            ...
            {authMode !== 'forgot' && (
              <>
                <div className="grid grid-cols-1 gap-3">
                  ...
                  <Button 
                    variant="ghost" 
                    className="w-full h-14 rounded-2xl bg-white/[0.01] border border-white/5 hover:bg-white/5 font-bold gap-3 text-white/60"
            ...
            <form onSubmit={handleEmailAuth} className="space-y-6">
              ...
              {authMode !== 'forgot' && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center px-4">
                    <Label className="text-[10px] uppercase tracking-widest text-white/40">Access Protocol</Label>
                    {authMode === 'login' && (
                      <button 
                        type="button"
              ...
              >
                {isLoading ? (
                  ...
                  authMode === 'login' ? "Access Studio" : authMode === 'signup' ? "Initialize Identity" : "Send Reset Email"
                )}
              </Button>
            </form>
          </div>
          ...
          <div className="text-center">
            {authMode === 'forgot' ? (
              ...
              >
                {authMode === 'login' ? "Need a neural identity? Sign Up" : "Already have an identity? Log In"}
              </button>
            )}
          </div>
        </CardContent>
